...
import { ShoppingCart, MessageCircle, Truck, RefreshCw, CreditCard, CheckCircle } from "lucide-react";
import type { Product, ProductColor } from "@/types";
import { formatPrice, formatDiscount, validateMoroccanPhone, sanitizeProductTitle } from "@/lib/utils";
import { generateCartItemId as cartId } from "@/lib/cart";
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <h1 className="text-2xl font-black text-brand-navy leading-tight">{sanitizeProductTitle(product.title, product.category_id)}</h1>
      ...
      {product.is_pack && product.pack_pieces && (
        <div className="bg-brand-gold/10 border border-brand-gold/30 p-3 rounded-sm">
          <p className="text-brand-navy font-bold text-sm">
            🎁 باك {product.pack_pieces} قطع — وفر {product.old_price ? formatPrice(product.old_price - product.price) : ""} مقارنة بالشراء بالقطعة
          </p>
        </div>
      )}
