...
export async function GET() {
  ...
  try {
    ...
    const { data } = await createAdminClient().from("blacklist").select("*").order("created_at", { ascending: false });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { createAdminClient } = await import("@/lib/supabase/server");
    const { data, error } = await createAdminClient().from("blacklist").insert(body).select().single();
